...
namespace pod {
	...
	PodPropeller::BladeSet::BladeSet(PodPropeller* propeller, const Types type, const double radius, const bool reversed, const double speed_ratio, const double theta_offset) :
		...
		for (int i = 0; i < description.count; i++) {
			Blade* blade = new Blade(description.blade_type, radius, 20.0 / 180.0 * PI, reversed);
			blade->Rotate(glm::dvec3(0, 0, 1), (reversed ? -1 : 1) * (theta_offset + (2.0 * PI) * i / description.count));
