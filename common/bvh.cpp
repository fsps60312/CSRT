...
BVH::BVH(const std::vector<glm::mat3>& triangles) {
...
void BVH::Build() {
	...
	Build(0, 0, (int)triangles.size() - 1);
...
void BVH::Build(const int id, const int l, const int r) {
	// range
	ranges[id] = glm::ivec2(l, r);
	// cal aabb
	auto& aabb = aabbs[id] = AABB();
	for (int i = l; i <= r; i++)aabb.AddTriangle(triangles[i]);
	if (r - l + 1 <= 2) {
		nodes[id].x = nodes[id].y = -1;
		return;
	}
	// split
	const int mid = CalMid(triangles, id);
	// dfs
...
int BVH::CalMid(std::vector<glm::mat3>& triangles, const int id)const {
	...
	std::sort(triangles.begin() + l, triangles.begin() + r + 1, [&c, &dot_max](const glm::mat3& t1, const glm::mat3& t2)->bool {
	...
	for (long long i = l; i < r; i++) {
		left_aabb.AddTriangle(triangles[i]);
	...
	for (long long i = r; i > l; i--) {
		rigt_aabb.AddTriangle(triangles[i]);
